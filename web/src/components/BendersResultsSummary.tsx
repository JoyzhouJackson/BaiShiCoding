...
export default function BendersResultsSummary({ data }: { data: ComparisonData }) {
  ...
  return (
    <section className="benders-result-summary" aria-labelledby="benders-results-title">
      <div className="subsection-title"><span id="benders-results-title">实验结果</span><small>12个案例与99个动态求解阶段</small></div>
      <div className="experiment-summary">
        ...
        <article><span>达到5%目标Gap</span><b>{convergence.gapReachedCount}/{convergence.phaseCount}</b><small>69个阶段上下界差距不超过5%</small></article>
      ...
      <p className="figure-footnote">其余{convergence.phaseCount - convergence.gapReachedCount}个阶段中，{convergence.innerTimeLimitCount}个触及内层时间上限，{convergence.stalledDuplicateCutCount}个因重复割停止；最大记录Gap为{(convergence.maxRecordedGap * 100).toFixed(2)}%。</p>
    </section>
  )
}
